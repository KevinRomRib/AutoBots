...
public class EmpControle {
	@Autowired
	private SelecionadorEmp selecionadorEmp;
	@Autowired
	...
	public ResponseEntity<Emp> obterEmp(@PathVariable long id) {
		...
		Emp emp = selecionadorEmp.selecionar(emps, id);
		if (emp == null) {
	...
	public ResponseEntity<?> updateEmp(@RequestBody Emp atualizacao) {
		HttpStatus status = HttpStatus.CONFLICT;
		Emp emp = repositorioEmpresa.getById(atualizacao.getId());
		if (emp != null) {
	...
	public ResponseEntity<?> deleteEmp(@RequestBody Emp exclusao) {
		...
		Emp emp = repositorioEmpresa.getById(exclusao.getId());
		if (emp != null) {
			repositorioEmpresa.delete(emp);
			status = HttpStatus.OK;
		}
