...
import com.autobots.automanager.selecter.UsuarioDocumentoSelecionador;
...
public class DocControle {
	...
	public ResponseEntity<Doc> obterDoc(@PathVariable long id) {
		...
		if (documento == null) {
			...
			for (Doc doc : docs) {
				User user = selecionadorUsuDocumento.selecionar(users, documento);
				adicionadorLinkDocumento.adicionarLink(doc, user);
			}
	...
	public ResponseEntity<List<Doc>> obterDoc() {
		...
		if (docs.isEmpty()) {
			...
			List<User> users = repositorioUsuario.findAll();
			for (Doc doc : docs) {
				User user = selecionadorUsuDocumento.selecionar(users, doc);
				adicionadorLinkDocumento.adicionarLink(doc, user);
			}
