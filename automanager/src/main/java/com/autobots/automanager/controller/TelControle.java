...
import com.autobots.automanager.repository.RepositorioTelefone;
import com.autobots.automanager.repository.RepositorioUsuario;
import com.autobots.automanager.selecter.SelecionadorTel;
import com.autobots.automanager.selecter.UsuarioTelefoneSelecionador;
...
public class TelControle {
	...
	public ResponseEntity<Tel> obterTel(@PathVariable long id) {
		...
		if (telefone == null) {
			...
			for (Tel tel : tels) {
				User user = selecionadorUsuTelefone.selecionar(users, telefone);
				adicionadorLinkTelefone.adicionarLink(tel, user);
			}
	...
	public ResponseEntity<List<Tel>> obterTel() {
		...
		if (tels.isEmpty()) {
			...
			List<User> users = repositorioUsuario.findAll();
			for (Tel tel : tels) {
				User user = selecionadorUsuTelefone.selecionar(users, tel);
				adicionadorLinkTelefone.adicionarLink(tel, user);
			}
